...
<div class="space-y-8 animate-fade-in">
  <header class="space-y-2">
    ...
    <p class="text-sm text-[rgb(var(--fg-muted))]">{$t('identityHint')}</p>
  </header>
  ...
  <div class="grid grid-cols-5 gap-6">
    ...
    <aside class="col-span-2 hidden lg:block">
      <div class="card sticky top-0 p-4 text-xs space-y-2">
        ...
        <div class="font-semibold">{name || $t('identityPreviewName')}</div>
        <div class="text-[rgb(var(--fg-muted))]">{role || $t('identityPreviewRole')}</div>
        <div class="mt-2 text-[rgb(var(--fg-muted))] whitespace-pre-wrap">
          {blurb || $t('identityPreviewBlurb')}
        </div>
      </div>
    </aside>
  </div>
